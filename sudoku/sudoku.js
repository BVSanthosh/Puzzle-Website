    ...
    return (
        <div className={styles.cell}>
            <input
                ...
                onFocus={(e) => {props.onFocus(props.c, props.r, e)}}
                onBlur={(e) => {props.onBlur(props.c, props.r, e)}}
            />
        </div>
    );
}
    ...
    renderTile(c, r, i, d) {
        return (
            <Tile
                ...
                onFocus={(c, r, e) => this.props.onFocus(c, r, e)}
                onBlur={(c, r, e) => this.props.onBlur(c, r, e)}
            />
        );
    }
    renderRow(a, b) {
...
class Sudoku extends React.Component {
    ...
    handleChange(c, r, event) {
        ...
        if (value == "" || value == null) { // Reset square to null if value removed.
            tempArr[c][r] = "";
        } else if (!isNaN(value) && value.length <= 1 && value != " ") { // If value is number set square to that number
            tempArr[c][r] = value;
        }
        ...
        if (solved && !this.state.createMode && !this.state.solved) {
            ...
            console.log('props:', this.props.user_id, this.props.puzzle_id);
    ...
    // Solve button (validate the puzzle)
    handleSolve() {
        ...
        if (tempBoard != null && tempBoard != "invalid") {
            ...
            alert("Validated Sudoku")
        }
        else {
            console.log(tempBoard);
            this.findMistakes(this.state.board);
            alert("Invalid Sudoku: must have exactly 1 solution");
        }
    }
        ...
        return (
            <div>
                ...
                <Board
                    ...
                    onFocus={(c, r, e) => this.highlight(c, r, e)}
                    onBlur={(c, r, e) => this.clear(c, r, e)}
                    pencil={this.state.pencil}
                />
                
                <div>
                    <button className={styles.pencilButton} onClick={() => this.handleClick1(1)}>1</button>
                    <button className={styles.pencilButton} onClick={() => this.handleClick1(2)}>2</button>
                    <button className={styles.pencilButton} onClick={() => this.handleClick1(3)}>3</button>
                </div>

                <div>
                    <button className={styles.pencilButton} onClick={() => this.handleClick1(4)}>4</button>
                    <button className={styles.pencilButton} onClick={() => this.handleClick1(5)}>5</button>
                    <button className={styles.pencilButton} onClick={() => this.handleClick1(6)}>6</button>
                </div>

                <div>
                    <button className={styles.pencilButton} onClick={() => this.handleClick1(7)}>7</button>
                    <button className={styles.pencilButton} onClick={() => this.handleClick1(8)}>8</button>
                    <button className={styles.pencilButton} onClick={() => this.handleClick1(9)}>9</button>
                </div>
                
                <button className={styles.mode} onClick={(e) => this.handleMode(e)}>Pencil Mode</button>

                {this.state.createMode ?
                    <div>
                        <button className={styles.button} onClick={() => this.handleSolve()}>Validate Sudoku</button>
                    </div>
                    : " "}
                ...
                {this.state.createMode ?
                    <div className={styles.create_save}>
                        ...
                        <br></br>
                        <Button variant="primary" onClick={() => this.handleSave()}>Save Sudoku</Button>
                    </div>
                    : ""}
    ...
    // Pencil markings
    highlight(c, r, e) {
        if (this.state.pencil == null) {
            this.setState({pencil: 0});
        }
        e.target.style.backgroundColor = "#d1a3e3";
        this.setState({highlightc: c});
        this.setState({highlightr: r});
    ...
    handleClick1(v) {
        this.state.target.value = v;
        if (this.state.pencil == 1){
            this.state.target.style.color = "grey";
        }
        if (this.state.pencil == 0){
            this.state.target.style.color = "black";
            let value = v;
            let c = this.state.highlightc;
            let r = this.state.highlightr;
            console.log("update c" + c + " r" + r + " val" + value);
            let tempArr = [];
            for (let i = 0; i < this.state.board.length; i++) { // Make a copy of the board to edit.
                tempArr.push(this.state.board[i].slice());
            ...
            if (value == "" || value == null) { // Reset square to null if value removed.
                tempArr[c][r] = null;
            } else if (value != " ") { // If value is number set square to that number
                tempArr[c][r] = value;
            }
    ...
    alertSolve() {
        Swal.fire({
            icon: 'success',
            title: 'Good job!',
            text: 'you solved this sudoku',
            showCancelButton: true,
            cancelButtonText: `return`,
            confirmButtonText: '<a style="text-decoration:none; color:white" href="../puzzle-list/sudoku">solve another</a>',
        })
    }
